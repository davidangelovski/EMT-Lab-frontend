...
const EditBookDialog = ({ book, open, onClose, onEdit }: EditBookDialogProps) => {
    ...
    useEffect(() => {
        ...
        const matchedAuthor = authors.find(
            (author) => author.name === book.authorName && author.surname === book.authorSurname
        );

        setFormData({
            name: book.name,
            category: book.category,
            authorId: matchedAuthor?.id.toString() ?? '',
            state: book.state,
                ...
                <Stack spacing={2} sx={{ pt: 1 }}>
                    ...
                    {authors.length === 0 && (
                        <Typography variant='body2' color='text.secondary'>
                            No authors available. Add authors first before editing books.
                        </Typography>
                    )}
                </Stack>
            </DialogContent>
